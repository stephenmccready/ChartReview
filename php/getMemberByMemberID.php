...
WHERE M.SourcePersonKey = '" . $_GET['memberid'] . "'";
...
if ($result->num_rows == 1)
{
	while($record = $result->fetch_assoc())
	{
		...
		echo $record['ID'] . '|' . $record['PersonLastName']. '|' . $record['PersonFirstName'] . '|' . $record['MBI'] . '|' . $record['HICN'] . '|' . date_format($date, "Y-m-d") . '|' . $record['MasterPersonID'];
	}
