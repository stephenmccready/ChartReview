...
WHERE MBI.SourcePersonKey = '" . $_GET['mbi'] . "'";
...
if ($result->num_rows == 1)
{
	while($record = $result->fetch_assoc())
	{
		...
		echo $record['ID'] . '|' . $record['PersonLastName']. '|' . $record['PersonFirstName'] . '|' . $record['MemberID'] . '|' . $record['HICN'] . '|' . date_format($date, "Y-m-d") . '|' . $record['MasterPersonID'];
	}	
} else {
	echo 0;
}
